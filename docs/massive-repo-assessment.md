...
For each check, decide BUG / EXPECTED / N-A using the discriminator, and cite the JSON counts and timings you actually read.
  ...
  8. ARTIFACTS OUTSIDE .humify/. After any `humify verify` or `humify plan` (non --no-coverage) run, check `git status --porcelain` for NEW files outside `.humify/` (stray build binary, coverage temp, coverage/ dir, __pycache__). Any such write is humify executing repo build/test code and leaking artifacts — record it as a half-(B) finding (an EXPECTED side effect of running project code, but a caveat on the read-only claim). Do NOT delete them.
